...
#   further information. Last update: August 10, 2016. For questions, please email       #
#   jcbagley@unb.br.                                                                     #
##########################################################################################
...
############ STEP #1: SETUP VARIABLES AND CHECK CONTENTS
...
echo "##########  Found $MY_NUM_RUN_FOLDERS run folders present in the current working directory. "


############ STEP #2: MAKE BASH SUBMSSION FILE; MOVE ALL RUN FOLDERS TO SUPERCOMPUTER; AND 
############ THEN CHECK BASH SUBMISSION FILE WAS CREATED
...
echo "##########  Starting copying run folders to supercomputer... note: this may display folder contents transferred rather than folder names."
(
for i in ./*/
	do
...
if [ -f ./raxmlrunner_batch_qsub.sh ]; then
    echo "##########  Batch queue submission file ("raxmlrunner_batch_qsub.sh") successfully created."
else
    echo "##########  Something went wrong. Batch queue submission file ("raxmlrunner_batch_qsub.sh") not created."
fi

############ STEP #4: MOVE BASH SUBMISSION FILE TO SUPERCOMPUTER AND EXECTUE IT!
echo "##########  Moving batch file to supercomputer and executing it..."
...
echo "##########  Finished copying run folders to supercomputer and submitting RAxML jobs to queue!!"
echo "##########  Bye."
#
#
#
