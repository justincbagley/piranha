...
USAGE="Usage: $(basename $0) [Help: -h help H Help] [Options: -i n w v V --version] [stdin:] <workingDir> 
 ## Help:
...
VERBOSE_USAGE="Usage: $(basename $0) [Help: -h help H Help] [Options: -i n w v V --version] [stdin:] <workingDir> 
 ## Help:
  -h   help text (also: --help) echo this help text and exit
  -H   verbose help text (also: -Help) echo verbose help text and exit
...
if [[ "$USER_SPEC_PATH" = "$(echo $(pwd))" ]] || [[ "$USER_SPEC_PATH" = "." ]]; then
	MY_WD_PATH="$(pwd -P)";
	echo "INFO      | $(date) |          Setting working directory to: $MY_WD_PATH "
elif [[ "$USER_SPEC_PATH" = ".." ]] || [[ "$USER_SPEC_PATH" = "../" ]] || [[ "$USER_SPEC_PATH" = "..;" ]] || [[ "$USER_SPEC_PATH" = "../;" ]]; then
	cd ..;
	MY_WD_PATH="$(pwd -P)";
	echo "INFO      | $(date) |          Setting working directory to: $MY_WD_PATH "
