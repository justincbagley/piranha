...
usage() {
 ...
 ${bold}Options:${reset}
  -d, --dir         Working directory, <workingDir>
 ...
 ${bold}Usage examples:${reset}
    $0 -d . -f calcAlignmentPIS -a -h         ## get help text for calcAlignmentPIS
    $0 -d . -f calcAlignmentPIS -a -t 150 .   ## run calcAlignmentPIS with threshold at 
                                                           N=150 alignments
...
while [[ $1 = -?* ]]; do
  case $1 in
    -d|--dir) USER_SPEC_PATH=$OPTARG ;;
    -f|--func) FUNCTION_TO_RUN=$OPTARG ;;
    -a|--args) FUNCTION_ARGUMENTS=$OPTARG ;;
